...
class TransformerAdapterLayer(nn.Module):
    ...
    def __init__(
        ...
        self.use_rotary_embeddings = use_rotary_embeddings
        self._init_submodules(add_bias_kv, use_esm1b_layer_norm)
        self.adapter_layer_dict = adapter_layer_dict
    ...
    def forward(
        ...
        outputs = []
        if self.adapter_layer_dict is not None:
          for module in self.adapter_layer_dict.values():
            one_output = (module[0](x) / len(self.adapter_layer_dict))
        ...
        if self.adapter_layer_dict is not None:
          for module in self.adapter_layer_dict.values():
            one_output = (module[1](x) / len(self.adapter_layer_dict))
            outputs.append(one_output)
          x = torch.sum(torch.stack(outputs), dim=0)
        x = residual + x
...
class RobertaLMHead(nn.Module):
    ...
    def __init__(self, embed_dim, output_dim, weight=None):
        super().__init__()
        self.dense = nn.Linear(embed_dim, embed_dim) #1280*1280 =
        self.layer_norm = ESM1bLayerNorm(embed_dim)
        if weight is None:
           self.weight = nn.Parameter(torch.randn(output_dim, embed_dim))
        else:
           self.weight = weight
        
        self.bias = nn.Parameter(torch.zeros(output_dim)) #33
    ...
    def forward(self, features):
        ...
        x = F.linear(x, self.weight) + self.bias  #1280*33
        return x
